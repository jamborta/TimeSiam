    ...
    def _find_non_neighours(self, x, t):
        ...
        if t > T / 2:
            t_n = np.random.randint(
                self.window_size // 2, max((t - self.delta + 1), self.window_size // 2 + 1), self.mc_sample_size
            )
        else:
            t_n = np.random.randint(
                min((t + self.delta), (T - self.window_size - 1)), (T - self.window_size // 2), self.mc_sample_size
            )

        x_n = torch.stack(
            [torch.from_numpy(x[:, t_ind - self.window_size // 2 : t_ind + self.window_size // 2]) for t_ind in t_n]
        )
...
class Dataset_PA(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.normalize:
            ...
            seq_x[:, 0:4] = ((seq_x[:, 0:4] - seq_x[0][0]) / seq_x[0][0]) * 100
            seq_y[:, 0:4] = ((seq_y[:, 0:4] - seq_y[0][0]) / seq_y[0][0]) * 100
