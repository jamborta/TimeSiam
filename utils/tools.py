...
def show_pretrain_case(past_window_true, future_window_true, future_window_mask, future_prediction, segment):
    ...
    for i in range(len(past_and_future_window) - 1):
        if i < len(past_window_true):
            axs.plot(
                [x[i], x[i + 1]],
                [past_and_future_window[i], past_and_future_window[i + 1]],
                "-",
                color="black",
                label="ture past window",
            )
            ...
            if past_and_future_window_mask[i] == 0:
                ...
                axs.plot(
                    ...
                    "-",
                    color="blue",
                    label="unmasked future window",
                )

            axs.plot(
                [x[i], x[i + 1]],
                [past_and_future_prediction[i], past_and_future_prediction[i + 1]],
                "-",
                color="orange",
                label="predition future window",
            )
...
def show_pretrain_case(
    past, current, masked_current, predict_current, segment, mask, noise=None, mask_rate=None, t=None
):
    ...
    for i in range(len(x) - 1):
        if i not in masked_idx:
            axs[1].plot(
                [x[i], x[i + 1]],
                [future_window_true[i], future_window_true[i + 1]],
                "-",
                color="blue",
            ...
            axs[1].scatter(
                x[i],
                future_window_mask[i],
                facecolors="none",
                edgecolors="red",
                linewidths=2,
                s=30,
                label=(
                    "{:.2f}% masked Gaussian noise, S/R ({:.2f}/{:.2f})".format(mask_rate, t[0], t[1])
                    if i == masked_idx[0]
                    else ""
                ),
            )
            ...
            axs[1].scatter(
                x[i], noise[i], facecolors="none", edgecolors="grey", linewidths=1, s=30, label="Gaussian noise"
            )
    ...
    for i in range(len(x) - 1):
        if i not in masked_idx:
            ...
            axs[1].scatter(
                ...
                linewidths=2,
                s=30,
                label="Masked timestamps" if i == masked_idx[0] else "",
            )
...
def show_pretrain_tst(pred, target, mask, mask_rate, name):
    ...
    for i in range(len(x) - 1):
        if i not in masked_idx:
            axs[0].plot(
                [x[i], x[i + 1]], [target[i], target[i + 1]], "-", color="blue", label="Remain time points", linewidth=2
            )
        else:
            axs[0].scatter(
                x[i],
                ...
                linewidths=2,
                s=30,
                label="{:.2f}% time points masking".format(mask_rate),
            )
...
def show_pretrain_patchtst(pred, target, mask, mask_rate, name):
    ...
    for i in range(len(x) - 1):
        if i in masked_idx:
            ...
            line_sty = "-"
        axs[0].plot(
            [x[i], x[i + 1]], [target[i], target[i + 1]], line_sty, color=color, label="Ground Truth", linewidth=2
        )
...
def show_mask(mask, title):
    ...
    print(
        "Masked: {0}\tUnmasked: {1}\tMasked ratio: {2:.2f}".format(true_count, false_count, true_count / mask.numel())
    )
    ...
    for t_i in range(time_points - 1):
        for pn in range(positive_numbers):
            ...
            if batch_x_m[s_i][t_i] == 0:
                axs[0].plot(
                    [x[t_i], x[t_i + 1]],
                    [batch_x[idx][t_i], batch_x[idx][t_i + 1]],
                    ":",
                    color="grey",
                    alpha=0.5,
                    label="masked",
        ...
        axs[1].plot(
            [x[t_i], x[t_i + 1]], [batch_x[idx][t_i], batch_x[idx][t_i + 1]], "-", color="blue", label="original"
        )
        axs[1].plot(
            [x[t_i], x[t_i + 1]],
            [pred_batch_x[idx][t_i], pred_batch_x[idx][t_i + 1]],
            "-",
            color="orange",
            label="prediction",
        )
...
class AggregationRebuild(torch.nn.Module):
    ...
    def forward(self, similarity_matrix, batch_emb_om):
        ...
        similarity_matrix = (
            similarity_matrix - torch.eye(cur_batch_shape[0]).to(similarity_matrix.device).float() * 1e12
        )
